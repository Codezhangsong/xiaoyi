...
class Activity extends Model
{
    ...
    {
        return DB::table('activity')->insertGetId($arr);
    ...
    static public function channel($arr)
    {
        $act = DB::table('activity_channel_detail')->where($arr['where'])->first();
        $url = $act->url;
        $act_id = $act->id;

        $channel = DB::table('activity_channel')->where('org_code',ORGCODE)->get();
        $count = $channel->count();
        if(!empty($channel)){
            foreach ($channel as $key => &$value)
            {
                $value->url = $url.'?'.base64_encode($act_id.$value->id);
            }
        }

        return ['data'=>$channel,'count'=>$count];
    }
