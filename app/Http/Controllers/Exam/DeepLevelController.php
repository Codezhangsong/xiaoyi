...
use Carbon\Carbon;
use App\Model\Exam\DeepLevel;

class DeepLevelController extends Controller
{
    ...
    public function add(Request $request)
    {
        ...
        try{
            ...
            //判断数据是否已存在
            if(DeepLevel::single(['name'=>$name])){
                return response()->json(['code'=>500,'msg'=>'数据已存在']);
            }
            $res = DeepLevel::add($arr);
            if(!$res)
