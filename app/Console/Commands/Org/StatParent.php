...
class StatParent extends Command
{
     ...
     * @var string
     */
    protected $signature = 'org:parent';
    ...
    public function handle()
    {
        ...
        try{
            $orgs = Orgs::get();
            if(empty($orgs)){
                throw new \Exception('暂无机构数据');
            }
            foreach ($orgs as $key=>$value)
            {
                $res = StatParents::where('stat_date',$today)->where('org_code',$value->org_code)->count();

                if($res){
                    continue;
                }else{
                    //家长总数
                    $parents = DB::table('parents')->where('org_code',$value->org_code)->count();

                    $sql  = "select COUNT(id) as num , city from ty_parents  where org_code = '$value->org_code' group by city order by num desc limit 1";
                    ...
                    //男女比例
                    $sql = "select count(id) as num ,gender from ty_parents where org_code = '$value->org_code' group by gender";
                    ...
                    $sql = "select age_temp,count(*) as total from (select age,
                        ...
                        from ty_parents where org_code = '$value->org_code'
